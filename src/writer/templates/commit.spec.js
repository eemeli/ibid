...
before(function (done) {
  fs.readFile(path.resolve(__dirname, 'commit.hbs'), function (err, data) {
    if (err) done(err)
...
describe('partial.commit', function () {
  it('should ignore host and owner if they do not exist and just use repository to link', function () {
    ...
    expect(log).to.equal(
      '* my header ([hash](www.myhost.com/a/b/my commits/hash)), closes [#1](www.myhost.com/a/b/my issue/1)\n'
    )
  })
  ...
  it('should just use repoUrl to link', function () {
    ...
    expect(log).to.equal(
      '* my header ([hash](www.myhost.com/my commits/hash)), closes [#1](www.myhost.com/my issue/1)\n'
    )
  })
  ...
  it('should link the commit if `linkReferences` is thuthy', function () {
    ...
    expect(log).to.equal(
      '* my header ([hash](www.myhost.com/a/b/my commits/hash))\n'
    )
  })
  ...
  it('should link reference if `references` is truthy and `linkReferences` is truthy', function () {
    ...
    expect(log).to.equal(
      '* my header ([hash](www.myhost.com/a/b/my commits/hash)), closes [#1](www.myhost.com/a/b/my issue/1) [#2](www.myhost.com/a/b/my issue/2) [#3](www.myhost.com/a/b/my issue/3)\n'
    )
  })
  ...
  it('should link reference from a different repository with an owner', function () {
    ...
    expect(log).to.equal(
      '* my header ([hash](www.myhost.com/a/b/my commits/hash)), closes [c/d#1](www.myhost.com/c/d/my issue/1)\n'
    )
  })
