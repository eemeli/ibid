...
function conventionalChangelogWriterInit(context, options) {
  ...
  if (
    !_.isBoolean(context.linkReferences) &&
  ...
  options = _.assign(
    {
      ...
      headerPartial: readFileSync(
        join(__dirname, 'templates/header.hbs'),
        'utf-8'
      ),
      ...
      footerPartial: readFileSync(
        join(__dirname, 'templates/footer.hbs'),
        'utf-8'
      )
    },
    options
  ...
  if (
    (!_.isFunction(options.transform) && _.isObject(options.transform)) ||
...
function conventionalChangelogWriterParseStream(context, options) {
  let generateOn
  ;({ context, options, generateOn } = conventionalChangelogWriterInit(
    context,
    options
  ))
  let commits = []
 ...
 */
conventionalChangelogWriterParseStream.parseArray = (
  rawCommits,
  context,
  options
) => {
  ...
  if (options.reverse) {
    entries.reverse()
    return (
      util.generate(options, commits, context, savedKeyCommit) +
      entries.join('')
    )
  } else {
    return (
